...
from nets.module import GaussConverter
...
class Trainer(Base):
    ...
    def add_model(self, stage='stage_one'):
        model = GaussConverter(self.model, stage)
...
class Tester(Base):
    ...
    def _make_model2(self):
        ...
        self.model = model
        self.add_model()
        self.model2.load_state_dict(ckpt['network'], strict=False)
        self.model2.eval()
...
class Transfer(Base):
    def __init__(self, test_epoch):
        super(Transfer, self).__init__(log_name = 'trans_logs.txt')
        self.test_epoch = int(test_epoch)

    def _make_batch_generator(self):
        # data load and construct batch generator
        self.logger.info("Creating dataset...")
        transet_loader = eval(cfg.dataset)(transforms.ToTensor(), 'transfer')
        batch_generator = DataLoader(dataset=transet_loader, batch_size=1, shuffle=False, num_workers=cfg.num_thread, pin_memory=True)
        
        self.transet = transet_loader
        self.batch_generator = batch_generator
        self.smplx_params = transet_loader.smplx_params
