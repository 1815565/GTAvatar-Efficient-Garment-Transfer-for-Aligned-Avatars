...
import torch
import numpy as np
from utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation, BasicPointCloud
...
class GaussianModel:
    ...
    def get_subject(self, id):
        rots = torch.zeros((self.get_xyz[id].shape[0], 4), device="cuda")
        rots[:, 0] = 1
        return {'mean_3d': self.get_xyz[id],
                'rgb': self.get_rgb[id],
                'radius': self.get_scaling[id],
                'opacity':torch.ones((self.get_xyz[id].shape[0], 1), dtype=torch.float, device="cuda"),
                'rotation': rots}
    ...
    def get_scaling(self):
        return self.scaling_activation(self._scaling.repeat(1, 1, 3))
    ...
    @property
    def get_rgb(self):
        return self.opacity_activation(self._features_dc)
    ...
    def get_covariance(self, id, scaling_modifier=1):
        if hasattr(self, 'rotation_precomp'):
            return self.covariance_activation(self.get_scaling[id], scaling_modifier, self.rotation_precomp)
        return self.covariance_activation(self.get_scaling[id], scaling_modifier, self._rotation[id])
    ...
    def get_opacity_loss(self):
        ...
        eps = 1e-6
        loss_opacity_cls = -(opacity * torch.log(opacity + eps) +
                             (1 - opacity) * torch.log(1 - opacity + eps)).mean()
        return {'opacity': loss_opacity_cls}

    def create_from_pcd(self, pcd_total: BasicPointCloud, spatial_lr_scale=1.):
        self.spatial_lr_scale = spatial_lr_scale

        
        if not isinstance(pcd_total, torch.Tensor):
            pcd_total = torch.tensor(np.asarray(pcd_total))
        colors = torch.ones_like(pcd_total)
        fused_point_cloud = pcd_total.float().cuda()
        fused_color = RGB2SH(colors.float().cuda())

        # # 初始值设为接近目标均值对应的x（sigmoid逆运算）
        # target_mean = ...  # 假设是0.5（中间值）
        # init_x = torch.log(target_mean / (1 - target_mean))  # 0.5对应x=0
        # # 加入小噪声，避免初始值完全相同（防止对称权重问题）
        # features = init_x + 0.1 * torch.randn_like(fused_color)  # 噪声范围缩小
        # self._features_dc = nn.Parameter(features.contiguous().requires_grad_(True))

        scale_list = []
        for i in range(len(fused_point_cloud)):
            dist2 = torch.clamp_min(distCUDA2(fused_point_cloud[i]), 0.0000001)
            scales = torch.log(torch.sqrt(dist2))[..., None] #.repeat(1, 1, 3)
            scale_list.append(scales)
        scale_list = torch.stack(scale_list, dim=0)

        features = torch.randn((fused_color.shape[0], fused_color.shape[1], 3)).float().cuda()
        # features[:, :, :3] = fused_color
        # features[:, :, 3:, 1:] = 0.0    

        print("Number of points at initialisation : ", fused_point_cloud.shape[0], '*', fused_point_cloud.shape[1])

        # self.fwd_transform = torch.randn(fused_point_clouds_stack.shape[0], fused_point_clouds_stack.shape[1], 4, 4)
        ...
        # opacities = torch.ones((fused_point_cloud.shape[0], fused_point_cloud.shape[1], 1), dtype=torch.float, device="cuda")
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self._features_dc = nn.Parameter(features[:,:,:].contiguous().requires_grad_(True))
        # self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        self._scaling = nn.Parameter(scale_list.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        self.xyz_gradient_accum = torch.zeros(
            (self.get_xyz.shape[0], self.get_xyz.shape[1], 1), device="cuda")
    ...
    def save_ply(self, path):
        ...
        elements = np.empty((xyz.shape[0], xyz.shape[1]), dtype=dtype_full)
        # attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation), axis=1)
        attributes = np.concatenate(
            (xyz, normals, opacities, scale, rotation), axis=2)

        b = list(map(tuple, attributes))
    ...
    def load_ply(self, path):
        ...
        xyz = np.stack((np.asarray(plydata.elements[0]["x"]),
                        ...
                        np.asarray(plydata.elements[0]["z"])),  axis=2)
        ...
        scales = np.zeros((xyz.shape[0], xyz.shape[1], len(scale_names)))
        ...
        rots = np.zeros((xyz.shape[0], xyz.shape[1], len(rot_names)))
        ...
        self._xyz = torch.tensor(
            xyz, dtype=torch.float, device="cuda").contiguous()
    ...
    #     new_features_dc = torch.empty(0)
    #     new_features_rest = torch.empty(0)
