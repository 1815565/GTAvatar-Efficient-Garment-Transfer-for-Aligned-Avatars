...
from .CrossAttn import SelfAttention, eca_block
...
class PoseVocab(nn.Module):
    def __init__(self,
        ...
        # self.self_attention_layer = SelfAttention(self.C * 3)
        self.eca_layer = eca_block(self.C * 3)
    ...
    def forward(self, id, query_points, scale, skinning_weight=None):
        ...
        feat_x, smooth_loss_x = self.sample(self.feat_lines_x,  yx[None,:,None,:], id)  # (B, N, J, C)
        feat_y, smooth_loss_y = self.sample(self.feat_lines_y,  zy[None,:,None,:], id)
        feat_z, smooth_loss_z = self.sample(self.feat_lines_z,  xz[None,:,None,:], id)
        ...
        if skinning_weight is None:
            ...
            # print(feat.shape)
            B, N, J, C = feat.shape
            feat = feat.reshape(B, N, J*C, 1).permute(0,2,1,3)
            feat = self.eca_layer(feat).permute(0,2,3,1).reshape(B, N, J*self.C*3)
        else:
            feat = torch.einsum("bnjc, nj -> bnc", torch.cat([feat_x, feat_y, feat_z], dim = 3), skinning_weight)
        
        # self.show()
        # exit()
        return feat, xyz_smooth_loss
