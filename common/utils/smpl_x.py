...
from obj_io import load_obj_mesh
import pickle
...
class SMPLX(object):
    def __init__(self):
        ...
        self.layer_arg = {'create_global_orient': True, 'create_body_pose': True, 'create_left_hand_pose': True, 'create_right_hand_pose': True, 'create_jaw_pose': True, 'create_leye_pose': True, 'create_reye_pose': True, 'create_betas': True, 'create_expression': True, 'create_transl': True}
        ...
        self.scalp_vertex_idx = self.get_hair_vertex_idx()
        self.neck_vertex_idx = self.get_neck()
        ...
        _, _, self.vertex_uv, self.face_uv = load_obj_mesh(osp.join(cfg.human_model_path, 'smplx', 'smplx_uv', 'smplx_uv.obj'), with_texture=True)
    ...
    def set_id_info(self, shape_param, face_offset, joint_offset, locator_offset):
        self.shape_param.append(shape_param)
        self.face_offset.append(face_offset)
        self.joint_offset.append(joint_offset)
        self.locator_offset.append(locator_offset)
    ...
    def get_hair_vertex_idx(self):
        # FLAME 2020 has all vertices of expr_vertex_idx. use FLAME 2019
        with open(osp.join(cfg.human_model_path, 'flame', 'FLAME_masks', 'FLAME_masks.pkl'), 'rb') as f:
            masks = pickle.load(f, encoding='latin1')
        scalp = list(set(list(masks['scalp']))) #+list(masks['forehead'])  +list(masks['neck']
        # scalp = list(masks['scalp'])
        scalp_vertex_idx = self.face_vertex_idx[scalp]
        return scalp_vertex_idx
    
    def get_neck(self):
        # FLAME 2020 has all vertices of expr_vertex_idx. use FLAME 2019
        with open(osp.join(cfg.human_model_path, 'flame', 'FLAME_masks', 'FLAME_masks.pkl'), 'rb') as f:
            masks = pickle.load(f, encoding='latin1')
        neck = list(set(list(masks['neck'])+list(masks['boundary'])))
        neck_vertex_idx = self.face_vertex_idx[neck]
        return neck_vertex_idx
    ...
    def get_hand_palm_back(self, mesh_neutral_pose, skinning_weight):
    # 获取网格的法线
        normal = Meshes(verts=mesh_neutral_pose[None, :, :], 
                        faces=torch.LongTensor(self.face_upsampled).cuda()[None, :, :]).verts_normals_packed().reshape(self.vertex_num_upsampled, 3).detach()
        
        # 获取关节点的标签
        part_label = skinning_weight.argmax(1)
        
        # 假设你已经有手部关节点的索引
        hand_joints = [ 'L_Index_1', 'L_Index_2', 'L_Index_3', 'L_Middle_1', 'L_Middle_2', 'L_Middle_3', 'L_Pinky_1', 'L_Pinky_2', 'L_Pinky_3', 'L_Ring_1', 'L_Ring_2', 'L_Ring_3', 'L_Thumb_1', 'L_Thumb_2', 'L_Thumb_3', # left hand joints
        'R_Index_1', 'R_Index_2', 'R_Index_3', 'R_Middle_1', 'R_Middle_2', 'R_Middle_3', 'R_Pinky_1', 'R_Pinky_2', 'R_Pinky_3', 'R_Ring_1', 'R_Ring_2', 'R_Ring_3', 'R_Thumb_1', 'R_Thumb_2', 'R_Thumb_3']  # 假设这些关节点存在
        is_hand = 0
        for name in hand_joints:
            is_hand = is_hand + (part_label == self.joints_name.index(name))
        is_hand = (is_hand > 0)
        
        # 计算手心和手背
        # 假设手心法线方向向下（y轴负方向），手背法线方向向上（y轴正方向）
        is_palm = is_hand * (normal[:, 1] <= math.cos(math.pi / 3))  # 法线向下
        is_back = is_hand * (normal[:, 1] > math.cos(math.pi / 3))   # 法线向上
        
        return is_palm, is_back


    def get_upper_body(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_upper_body = 0
        for name in ('Pelvis', 'Spine_1', 'Spine_2', 'Spine_3', 'L_Collar', 'R_Collar', 'R_Shoulder', 'L_Shoulder', ):   # , 'Neck' 'L_Elbow', 'R_Elbow'
            is_upper_body = is_upper_body + (part_label == self.joints_name.index(name))
        is_upper_body = (is_upper_body > 0)
        
        return is_upper_body
    
    def get_lower_body(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_lower_body = 0 
        for name in ('L_Hip', 'R_Hip', 'L_Knee', 'R_Knee'):  #  'Pelvis'
            is_lower_body = is_lower_body + (part_label == self.joints_name.index(name))
        is_lower_body = (is_lower_body > 0)
        
        return is_lower_body
    
    def get_foot(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_foot = 0 
        for name in ('L_Foot', 'R_Foot', 'L_Ankle', 'R_Ankle'):
            is_foot = is_foot + (part_label == self.joints_name.index(name))
        is_foot = (is_foot > 0)
        
        return is_foot
    
    
    def get_head(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_eyes = 0
        for name in ('Head', 'Jaw', 'L_Eye', 'R_Eye', 'Neck'):
            is_eyes = is_eyes + (part_label == self.joints_name.index(name))
        is_eyes = (is_eyes > 0)
        
        return is_eyes
    
    def get_face(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_eyes = 0
    ...
    def get_eyes(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_eyes = 0
    ...
    # def get_neck(self, skinning_weight):
    #     part_label = skinning_weight.argmax(1)
    #     is_neck = 0
    #     is_neck = is_neck + (part_label == self.joints_name.index('Neck'))
    #     is_neck = (is_neck > 0)
        
    #     return is_neck
    
    def get_body_part(self, skinning_weight, i):
        part_label = skinning_weight.argmax(1)
        mask = 0
        mask = mask + (part_label == i)
        mask = (mask > 0)
        
        return mask
    
    def divide_parts(self, skinning_weight):
        part_label = skinning_weight.argmax(1)
        is_body = 0
        for name in ('Pelvis', 'Spine_1', 'Spine_2', 'Spine_3', 'Neck', 'Head', 'Jaw', 'L_Eye', 'R_Eye'):
            is_body = is_body + (part_label == self.joints_name.index(name))
        is_body = (is_body > 0)

        is_lhand = 0
        for name in ('L_Collar', 'L_Shoulder', 'L_Elbow', 'L_Wrist', 'L_Index_1', 'L_Index_2', 'L_Index_3', 'L_Middle_1', 'L_Middle_2', 'L_Middle_3', 'L_Pinky_1', 'L_Pinky_2', 'L_Pinky_3', 'L_Ring_1', 'L_Ring_2', 'L_Ring_3', 'L_Thumb_1', 'L_Thumb_2', 'L_Thumb_3'):
            is_lhand = is_lhand + (part_label == self.joints_name.index(name))
        is_lhand = (is_lhand > 0)

        is_rhand = 0
        for name in ('R_Collar', 'R_Shoulder', 'R_Elbow', 'R_Wrist', 'R_Index_1', 'R_Index_2', 'R_Index_3', 'R_Middle_1', 'R_Middle_2', 'R_Middle_3', 'R_Pinky_1', 'R_Pinky_2', 'R_Pinky_3', 'R_Ring_1', 'R_Ring_2', 'R_Ring_3', 'R_Thumb_1', 'R_Thumb_2', 'R_Thumb_3'):
            is_rhand = is_rhand + (part_label == self.joints_name.index(name))
        is_rhand = (is_rhand > 0)

        is_lleg = 0
        for name in ('L_Hip', 'L_Knee', 'L_Ankle', 'L_Foot'):
            is_lleg = is_lleg + (part_label == self.joints_name.index(name))
        is_lleg = (is_lleg > 0)

        is_rleg = 0
        for name in ('R_Hip', 'R_Knee', 'R_Ankle', 'R_Foot'):
            is_rleg = is_rleg + (part_label == self.joints_name.index(name))
        is_rleg = (is_rleg > 0)

        return is_body, is_lhand, is_rhand, is_lleg, is_rleg
