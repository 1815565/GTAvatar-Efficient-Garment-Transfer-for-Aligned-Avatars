...
for subject_id, (outfit, outfit_datadir) in enumerate(data_dirs.items()):
    ...
    with open(osp.join(outfit_datadir, 'frame_list_test.txt')) as f: # train_split.txt
        lines = f.readlines()
    for line in lines:
        frame_idx = int(line)
        ...
        out_path = osp.join(output_path,  '%d.png' % frame_idx)
        out = cv2.cvtColor(cv2.imread(out_path), cv2.COLOR_BGR2RGB)
        
        
        # gt image
        gt_path = osp.join(outfit_datadir, 'frames', '%d.png' % frame_idx)
        # gt = cv2.imread(gt_path)[:,:,::-1]/255.
        gt = cv2.cvtColor(cv2.imread(gt_path), cv2.COLOR_BGR2RGB)
        
        # gt mask
        mask_path = osp.join(outfit_datadir, 'masks', '%d.png' % frame_idx)
