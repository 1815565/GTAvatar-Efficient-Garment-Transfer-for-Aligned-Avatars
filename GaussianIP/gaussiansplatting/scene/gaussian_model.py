...
class GaussianModel:
    ...
    def get_scaling(self):
        ...
        # if self._scaling.shape[1] == 3:
        #     return torch.clamp(self.scaling_activation(self._scaling), max=0.005) #.repeat(1, 3)
        # elif self._scaling.shape[1] == 1:
        #     return torch.clamp(self.scaling_activation(self._scaling).repeat(1, 3), max=0.001)
        # else:
        #     raise ValueError("Scaling should be either a vector of length 1 or 3.")
    ...
    def create_from_pcd(self, pcd_total: BasicPointCloud, spatial_lr_scale=1.):
        ...
        features = torch.rand((fused_color.shape[0], 3, 1)).float().cuda()
        ...
        self._features_dc = nn.Parameter(features[:,:,:].transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True)
        # self._scaling = nn.Parameter(scales.requires_grad_(True))
        self._rotation = rots
        # self._rotation = nn.Parameter(rots.requires_grad_(True))
        self._opacity = torch.ones(fused_point_cloud.shape[0], 1).cuda()
    ...
    def construct_list_of_attributes(self):
        ...
        #     scale = self._scaling.repeat(1, 3)
        # else:    
        #     scale = self._scaling.detach().cpu().numpy()
        dist2 = torch.clamp_min(distCUDA2(self.get_xyz), 0.0000001)
        scale = torch.sqrt(dist2)[..., None].repeat(1, 3).detach().cpu().numpy()
