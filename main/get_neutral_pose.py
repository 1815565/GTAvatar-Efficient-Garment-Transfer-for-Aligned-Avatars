    ...
    for subject_id, (outfit, outfit_datadir) in enumerate(data_dirs.items()):
        ...
        for i in range(view_num):
            ...
            with torch.no_grad():
                ...
                human_render = tester.model.module.gaussian_renderer(human_asset, render_shape, cam_param_rot, return_extra=True)
            ...
            for j in range(5):
                cv2.imwrite(osp.join(save_path, str(subject_id) + '_' + str(i) + f'_{j}' + '.png'), human_render['img_collect'][j].cpu().numpy().transpose(1,2,0)[:,:,::-1]*255)
