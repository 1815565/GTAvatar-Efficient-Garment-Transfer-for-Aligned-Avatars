...
from base import Tester, Transfer
import os
...
def main():
    ...
    for subject_id, (outfit, outfit_datadir) in enumerate(data_dirs.items()):
        ...
        for i in range(view_num):
            ...
            with torch.no_grad():
                human_asset, human_asset_refined = transfer.model.module.human_gaussian.transfer(subject_id, 0, 1, smplx_param, cam_param, ['upper_body', 'lower_body'])
                human_render = transfer.model.module.gaussian_renderer(human_asset, render_shape, cam_param_rot, return_extra=False)
