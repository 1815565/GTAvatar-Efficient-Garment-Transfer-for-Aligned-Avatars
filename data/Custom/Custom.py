...
class Custom(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.root_path = osp.join('..', 'data', 'Custom', 'data')
    ...
    def load_data(self):
        ...
        for subject_id, (outfit, outfit_datadir) in enumerate(self.data_dirs.items()):
            ...
            # read split file
            frame_idx_list = []
            if cfg.fit_pose_to_test or self.data_split=='transfer': #or self.data_split=='test'
                split_path = osp.join(outfit_datadir, 'frame_list_train.txt')
            else:
                split_path = osp.join(outfit_datadir, 'frame_list_train.txt')
            ...
            cam_params = {}
            if osp.isfile(osp.join(outfit_datadir, 'sparse', 'cameras.txt')) and osp.isfile(osp.join(outfit_datadir, 'sparse', 'images.txt')):
            ...
            elif len(glob(osp.join(outfit_datadir, 'cam_params', '*.json'))) > 0:
                print('Load virtual camera parameters')
                self.scene_from_colmap = False
                cam_param_path_list = glob(osp.join(outfit_datadir, 'cam_params', '*.json'))
            ...
            img_paths = {}
            # load image paths
            img_path_list = glob(osp.join(outfit_datadir, 'frames', '*.png'))
            ...
            for idx, frame_idx in enumerate(frame_idx_list):
                ...
                # masked_img = img * mask
                # torchvision.utils.save_image(masked_img, os.path.join(outfit_datadir, f"render_{idx}.png"))
            ...
            for smplx_param_path in smplx_param_path_list:
                frame_idx = str(smplx_param_path.split('/')[-1][:-5])
